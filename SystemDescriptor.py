...
def set_logger(path):
    ...
    hdlr = logging.FileHandler(path + '/result_SysDesc.log')
    formatter = logging.Formatter('%(asctime)s %(levelname)s %(message)s')
    hdlr.setFormatter(formatter)
    logger.addHandler(hdlr)
    logger.setLevel(logging.INFO)
    open(path + '/result_SysDesc.log', 'w').close()
    return logger


# check compatibility between interfaces or types
def check_compatibility(path_list, file_list, output_path, disableCheck, logger):
    info_no = 0
    warning_no = 0
    error_no = 0
                ...
                if index1 != index2:
                    if types[index1]['SHORT-NAME'] == types[index2]['SHORT-NAME']:
                        if types[index1]['VERSION'][-12:] != types[index2]['VERSION'][-12:]:
                            if disableCheck:
                                logger.warning('The interface: ' + types[index1]['SHORT-NAME'] + ' has different versions:' + types[index1]['VERSION'] + ' and ' + types[index2]['VERSION'])
                                print('The interface: ' + types[index1]['SHORT-NAME'] + ' has different versions:' + types[index1]['VERSION'] + ' and ' + types[index2]['VERSION'])
                                warning_no = warning_no + 1
                            else:
...
def generate_system(path_list, file_list, output_path, system_name, mapping_name, composition_name, modularity, serviceTable, logger):
    ...
    if output_path:
        if os.path.isdir(output_path):
            if output_log:
                ...
                if modularity:
                    generation = check_compatibility(path_list, file_list, output_path, disableCheck, logger)
                if generation:
                    generate_system(path_list, file_list, output_path, system_name, mapping_name, composition_name, modularity, serviceTable, logger)
            else:
        ...
        if output_arxml:
            if os.path.isdir(output_arxml):
                if output_log:
                    ...
                    logger = set_logger(output_log)
                    if modularity:
                        generation = check_compatibility(path_list, file_list, output_arxml, disableCheck, logger)
                    if generation:
                        generate_system(path_list, file_list, output_arxml, system_name, mapping_name, composition_name, modularity, serviceTable, logger)
                else:

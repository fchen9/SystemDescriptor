...
def check_compatibility(recursive_dico, simple_dico, output_path, logger):
    info_no = 0
    warning_no = 0
    error_no = 0
                    ...
                    if system_version[elem]['ROOT'] == system_version[elem2]['ROOT']:
                        if system_version[elem]['SYSTEM'] == system_version[elem2]['SYSTEM']:
                            if version != system_version[elem2]['DATA']:
                                ...
                                print('Multiple <SYSTEM-VERSION> values: ' + version + ' <-> ' + system_version[elem2]['DATA'] + " for " + system_version[elem]['ROOT'] + '/' + system_version[elem]['SYSTEM'])
                                error_no = error_no + 1
                            else:
...
def main():
    ...
    generation = True
    if modularity:
        generation = check_compatibility(recursive_path_dico, simple_path_dico, sys_path, logger)
    if generation:
